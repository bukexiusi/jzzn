...
from service.client.document_create import *
from service.client.document_create_new import document_new
from service.client.document_create_third import document_third
...
class TaskExecution():
    ...
    def __init__(self, driver, task, resource, executeIndex, executeIndexStatus):
        self._driver = driver
        self._task = task
        ...
        self._widgetDetail = {}
        self._executeIndex = executeIndex
        self._executeIndexStatus = executeIndexStatus
    ...
    def _getCompareRes(self, condition, currentWidgetSet):
        ...
        leftValue = leftValue if leftValue else condition.get('value_left')
        ...
        if outerKeyRight:
            ...
            if tempWidget:
                collect_value_type = tempWidget.get('collect_value_type')
            collect_value_type = collect_value_type if collect_value_type else 'TEXT'
        rightValue = rightValue if rightValue else condition.get('value_right')

        # 比较
        print('leftValue=', leftValue, 'rightValue=', rightValue)
        flag = self._compare_value(leftValue, rightValue, symbol, collect_value_type)
        return flag
    ...
    def _getBrowserLeftValue(self, condition, currentWidgetSet):
        ...
        if symbol in totalArray:
            ...
            # if isinstance(pageValues, dict):
            vv = pageValues.get(widgetName)  # 控件
            # for kk, vv in pageValues.items():
            ...
            parent_idx = currentWidgetSet.get('parent_idx')  # 页面索引
            result = browserValues.get(parent_idx)
            if not result:
                # 解决多对一的关系
                result = browserValues.get('0')
            return result
    ...
    def _getWidgetValue(self, input_condition, page_index, row_index):
        ...
        widget_set_value = collect_value.get(input_condition.get("widget_set_id_right"))
        if not widget_set_value:
            return ''
        widget_value = widget_set_value.get(input_condition.get("widget_id_right"))
        if not widget_value:
            return ''
...
class StepExecution():
    ...
    # 切换iframe窗口
    def switchIframe(self, iframeStr):
        ...
        if currentIframe == iframeStr:
            ...
            if not iframeStr:
                ...
                if currentIframe and iframeStr.startswith(currentIframe):
                    ...
                    self.__switchIframeCircle(iframeStr)
                else:
                    self._driver.switch_to_default_content()
                    self.__switchIframeCircle(iframeStr)
    ...
    # 循环切换iframe
    def __switchIframeCircle(self, iframeStr):
        # ;号分割，格式 1;2;3...
        ...
        for iframe in iframes:
            try:
                ...
                temp = iframe
            self.__switch_to_frame_wait(temp)
            print('控件集iframe切换：=============', iframe)

    # 等待iframe并切换
    def __switch_to_frame_wait(self, ID, WaitTime=30, WaitFq=0.1):
        t = time.time()
        while True:
            f = 0
            try:
...
class WidgetSetExecution():
    ...
    def executeWidgetSet(self):
        # 控件集类型
        # widget_set_type = widget_set.get('widget_set_type')
        # 采集箱类型
        # if (widget_set_type == 'collect_set'):
        # 采集箱循环
        self.__executeWidgetSetPageCircle()
        # else:  # 动作集类型
        # 动作集循环
        # self.__executeWidgetSetResourceCircle(widget_set)
    ...
    def __setWidgetSetPageIndex(self, index=0):
        if self._stepExecution._parentWidgetExecution:
            ...
            parentPage = parentWidgetSet.get('is_exist_turn_page')
            if parentPage:  # 父层级存在翻页
                self._currentWidgetSet['turn_page_index'] = parentWidgetSet.get('turn_page_index') + '-' + str(index)
            else:
    ...
    def __executeNexPage(self):
        ...
        element = WebDriverWait(self._driver, 30, 0.1).until(
    ...
    def __switchWigetSetPageIframe(self):
        iframeStr = self._currentWidgetSet.get('turn_page_iframe')  # 切换iframe
        self._stepExecution.switchIframe(iframeStr)
    ...
    def __executeWidgetSetElementCircle(self):
        ...
        if is_collect_single:  # 单条采集
            self.__setWidgetSetBrowserIndex()
    ...
    def __executeWidgetSetResourceCircle(self):
        ...
        self._currentWidgetSet["row_index"] = '0'
        for idx in range(circle.get('times')):
            self.__executeWidgetSetResource()
            row_index = int(self._currentWidgetSet["row_index"]) + 1
            self._currentWidgetSet["row_index"] = str(row_index)
    ...
    def __circleExecuteBrowserElements(self):
        ...
        # 所有元素
        elements = WebDriverWait(self._driver, 30, 0.1).until(EC.visibility_of_all_elements_located((By.XPATH, xpath)))
        print('页面循环次数：', len(elements))
        # cycle_index_step = int(self._currentWidgetSet.get("cycle_index_step")) if self._currentWidgetSet.get("cycle_index_step") else 1
        ...
        for idx in range(len(elements)):
            self._currentWidgetSet['parent_current_idx'] = idx
            self.__setWidgetSetBrowserIndex(self._currentWidgetSet['parent_current_idx'])
    ...
    # 设置页面索引
    def __setWidgetSetBrowserIndex(self, index=0):
        if self._stepExecution._parentWidgetExecution:
            parentWidgetSet = self._stepExecution._parentWidgetExecution._widgetSetExecution._currentWidgetSet
            parentBrowser = parentWidgetSet.get('is_collect_single')
            if parentBrowser:  # 父层级循环
                self._currentWidgetSet['parent_idx'] = parentWidgetSet.get('parent_idx') + '-' + str(index)
            else:
                self._currentWidgetSet['parent_idx'] = str(index)
        else:
            self._currentWidgetSet['parent_idx'] = str(index)

    # 切换页面iframe
    def __switchWidgetSetBrowserIframe(self):
        iframeStr = self._currentWidgetSet.get('parent_iframe')
        self._stepExecution.switchIframe(iframeStr)
    ...
    def __widgetPretreatment(self):
        ...
        for widget in widgets:
            ...
            if actionType == '5' or actionType == '6':
    ...
    def __executeWidgetSetResource(self):
        ...
        for idx in range(len(widgets)):
            ...
            try:
                ...
                # 强制执行
                force_execute = widget.get("force_execute")
                if not force_execute:
                    # 非强制执行 记录日志且抛错终止执行
                    error_log = widget.get("error_log") if widget.get("error_log") else ''
...
class WidgetExecution():
    ...
    def __collectAlertValue(self):
        ...
        if isAlert and actionType != '13':
            ...
            # 弹窗文本
            alert_text = alert.text
            print('alert内容：', alert_text)
            temp = {self._currentWidget.get('control_name'): {'0': {'0': alert_text}}}
            self._taskExecution._widgetValues[self._widgetSetExecution._currentWidgetSet.get('widget_set_name')] = temp
    ...
    # 切换采集元素iframe窗口
    def __switchCollectIframe(self):
        iframeStr = self._currentWidget.get('collect_iframe')
        self._stepExecution.switchIframe(iframeStr)

    # 切换动作iframe窗口
    def __switchActionIframe(self):
        # 判断是否有iframe   格式 1;2;3...
        iframeStr = self._currentWidget.get('action_iframe')
        self._stepExecution.switchIframe(iframeStr)
    ...
    def __getFullXpath(self, current_xpath, input_value):
        ...
        if parent_xpath:
            # parent_idx = widget_set.get('parent_idx')
            current_xpath = parent_xpath + '[' + str(
                self._widgetSetExecution._currentWidgetSet.get('parent_current_idx') + 1) + ']' + current_xpath[1:]
    ...
    def __handleActionWidget(self):
        ...
        if xpath:
            ...
            self.__switchActionIframe()
            xpath = self.__getFullXpath(xpath, inputValue)
            print('动作xpath===========', xpath)
            # if actionType == '2' or actionType == '7' or actionType == '8':
            #     element = WebDriverWait(self._driver, 30, 0.1).until(EC.element_to_be_clickable((By.XPATH, xpath)))
            # else:
            ...
            element = WebDriverWait(self._driver, timeout, 0.1).until(
        ...
        if actionType == '1':
            ...
            collect_data = execute_document.collect_data_to_data_format(data_source)
            params = {
                "data_type": "1",
                "task_id": self._currentWidget.get("task_id"),
                "ids": [self._taskExecution._resource.get("key")]
            }
            zn, colum_data = datatableservice.find_table_value_by_group_id(params)
            con = {'inputValue': inputValue, 'currentWidget': self._currentWidget, 'self_': self,
                   "collect_data": collect_data,
                   "extra_data": json.dumps(self._taskExecution._resource.get("document_data")),
                   "document_data": json.dumps(colum_data)}
            ...
            zn, colum_data = datatableservice.find_table_value_by_group_id(params)
            if len(colum_data) > 0:
                colum_data = colum_data[0]
            document_data = self._taskExecution._resource.get("document_data")
            for cdk, cdv in colum_data.items():
                document_data[cdk] = cdv
            json_ = {"driver": self._driver, 'inputValue': inputValue, "self_": self, "collect_data": collect_data,
            ...
            #                config.PROJECT_IP_PORT, str(self._taskExecution._executeIndexStatus))
            params = {
                'p1': 'C:/seleniumNode/Python36/pythonw.exe',
                'p2': 'C:/upload/runtime_handle.py',
                'p3': 'main_client',
                ...
                'p7': config.PROJECT_IP_PORT,
                'p8': self._taskExecution._executeIndexStatus
            }
            # 参数校验
            for param_k, param_v in params.items():
                if not param_v:
...
def main():
    ...
    def __closeAlert(self):
        # 弹窗标识
        isAlert = self._currentWidget.get('is_alert')  # 是否alert弹窗
        closeAlert = self._currentWidget.get('close_alert')  # 是否关闭alert弹窗
        actionType = self._currentWidget.get('action_type')
        # 采集alert弹窗内容
        collectAlert = True
        print('isAlert：', isAlert, 'isCloseAlert：', closeAlert)
        if isAlert and closeAlert and collectAlert and actionType != '13':
            alert = WebDriverWait(self._driver, 1000, 0.1).until(EC.alert_is_present())
            # 弹窗文本
            alert_text = alert.text
            print('_openedWindows:', self._taskExecution._openedWindows)
            print('alert内容：', alert_text)
            # 关闭弹窗
            alert.accept()
            time.sleep(1.5)
            # if True:  # TODO 一次点击事件出现多个alert弹出框-------------
            #     alert1 = WebDriverWait(self._driver, 8, 0.1).until(EC.alert_is_present())
            #     if alert1:
            #         alert1.accept()
        else:
            pass
        ...
        if self._currentWidget.get('close_window_self'):
            while True:
                ...
                if len(self._driver.window_handles) < len(
                        self._taskExecution._openedWindows) or time.time() - starttime > 30:
                    break
            # 最后一个窗口句柄出队
    ...
    def __getInputConditionValue(self):
        ...
        if condition_values:
            ...
            if input_condition:
                ...
                if resValue:
                    # 判断是否有正则截取
                    if input_condition.get('logical_expression'):
    ...
    def __collectBrowserValue(self):
        ...
        if timeout:
            ...
            timeout = 30
        timeout = timeout if timeout > 0 else 30
        element = WebDriverWait(self._driver, timeout, 0.1).until(EC.visibility_of_element_located((By.XPATH, xpath)))
    ...
    def __setBrowserValue(self, browser_value):
        ...
        # 控件id
        id = self._currentWidget.get('control_name')
        widgetValues = pageValues.get(id)  # 分页数组
        if not widgetValues:
            widgetValues = {}
            pageValues[id] = widgetValues
        ...
        parent_idx = self._widgetSetExecution._currentWidgetSet.get('parent_idx')  # 页面索引
        browserValues[parent_idx] = browser_value
