...
# from service.client.document_create_v31 import document_third
    ...
    def __executeWidgetSetResource(self):
        ...
        for idx in range(len(widgets)):
            ...
            try:
                ...
                error_log = widget.get("error_log")
                if error_log:
...
class WidgetExecution():
    ...
    def __handleActionWidget(self):
        ...
        if xpath:
            ...
            self._taskExecution._log = self._taskExecution._log + "动作xpath：" + xpath + "<br/>"
        ...
        if actionType == '1':
            ...
            #                config.PROJECT_IP_PORT, str(self._taskExecution._executeIndexStatus), data_dealt)
            params = {
                'p1': 'C:/seleniumNode/Python36/pythonw.exe',
                'p2': 'C:/upload/runtime_handle.py',
                'p3': 'main_client',
...
def main():
    ...
    def __collectBrowserValue(self):
        ...
        self._taskExecution._log = self._taskExecution._log + "采集xpath：" + xpath + "<br/>"
        # 超时时间
        timeout = self._currentWidget.get('timeout')
        if timeout:
            timeout = int(timeout)
