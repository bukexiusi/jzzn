...
if __name__ == '__main__':
    ...
    word = None
    try:
        word = Word() # 在线文档无法和本地文档处于同一个进程中
        word.open_document("D:/案件模板/1.docx")
    finally:
        word.rename_document("D:/案件模板/2.docx")
        if word:
            word.close_document()
            word.quit()
    '''
    ...
    try:
        word = Word()  # 在线文档无法和本地文档处于同一个进程中
        word.open_document("D:/1.doc")
        aString = windll.user32.LoadImageW(0, "D:/曾国量.bmp", win32con.IMAGE_BITMAP,
                                           0,
                                           0, win32con.LR_LOADFROMFILE)
        # print(aString)
        if aString != 0:  # 由于图片编码问题  图片载入失败的话  aString 就等于0
            ...
            win32clipboard.SetClipboardData(win32con.CF_BITMAP, aString)
            win32clipboard.CloseClipboard()
        word.word.Selection.Find.Execute('sdfsadf', False, False, False, False, False, True, 1, True,'^c', 2)
    except Exception as e:
        pass
    finally:
        word.close_document()
    ...
    # try:
    #     word = Word()  # 在线文档无法和本地文档处于同一个进程中
    #     word0 = word.open_document("D:/1.doc")
    # finally:
    #     print("0" + str(word0))
    #     word1 = word.close_document()
    #     print("1" + str(word1))
    #     word2 = word.quit()
    #     print("2" + str(word2))